...
class NotificationService {
  static async sendExpoNotification(expoPushToken, notification, data) {
    try {
      const message = {
        ...
        data: data,
        priority: 'high',
        channelId: data.messageType === 'BID_ACCEPTED' ? 'bid-updates' : 'chat-messages',
      };
  ...
  static async sendChatNotification(senderId, receiverId, messageContent, chatId) {
    try {
      // Get sender details
      ...
      if (!receiver || !receiver.deviceTokens || receiver.deviceTokens.length === 0) {
      ...
      const notification = {
        ...
        body: messageContent.messageType === 'BID_ACCEPTED' ? 
          '🤝 New bid accepted!' : 
      ...
      const results = await Promise.all(
        tokens.map(async token => {
          try {
            // Check if it's an Expo push token (starts with 'ExponentPushToken[' or 'ExpoPushToken[')
      ...
      const invalidTokens = tokens.filter((token, index) => {
        ...
        return error && (
          ...
          error.message?.includes('ExpoPushToken') // Handle Expo token errors
        );
      });
  ...
  static async sendBidAcceptedNotification(bid) {
    try {
      ...
      const notification = {
        ...
        body: `${sender.name} has accepted your bid of ₹${bid.biddedAmount.total} for the ${bid.bidType === 'LOAD_BID' ? 'load' : 'truck'} from ${bid.source.placeName} to ${bid.destination.placeName}`,
      };
  ...
  static async sendBidPlacedNotification(bid, loadPost) {
    try {
      ...
      const notification = {
        ...
        body: `${sender.name} has placed a bid of ₹${bid.biddedAmount.total} for your ${bid.bidType === 'LOAD_BID' ? 'load' : 'truck'} from ${bid.source.placeName} to ${bid.destination.placeName}`,
      };
    ...
    try {
      ...
      const notification = {
        ...
        body: status === 'ACCEPTED' ?
          `Your bid of ₹${bid.biddedAmount.total} for the ${bid.bidType === 'LOAD_BID' ? 'load' : 'truck'} from ${bid.source.placeName} to ${bid.destination.placeName} has been accepted!` :
          `Your bid of ₹${bid.biddedAmount.total} for the ${bid.bidType === 'LOAD_BID' ? 'load' : 'truck'} from ${bid.source.placeName} to ${bid.destination.placeName} was not accepted.`,
      };
